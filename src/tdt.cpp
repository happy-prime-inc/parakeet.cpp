...
std::vector<int32_t> tdt_decode_frames(
    ...
    int blank_id, int max_symbols, std::vector<TokenInfo>* tokens) {
    assert((int)enc.size() == (size_t)T * enc_hidden);
    assert(!durations.empty());

    const int num_dur = (int)durations.size();
    const int token_count = joint.V_plus() - num_dur;
    ...
    int t = std::min(st.pending_skip, T);
    st.pending_skip -= t;
    if (t == T) {
        st.frames_seen += T;
    ...
    while (t < T) {
        ...
        while (need_loop && symbols_added < max_symbols) {
            if (!g_valid) {
                const int32_t label = st.have_token ? st.last_token : blank_id;
                pred.step(label, !st.have_token, st.state, g, out_state);
                g_valid = true;
            }
            ...
            if (k != blank_id) {
                ...
                if (tokens) {
                    ...
                    tokens->push_back(TokenInfo{(int32_t)k,
                        (int32_t)(st.frames_seen + t), conf, (int32_t)skip});
                }
        ...
        if (skip == 0) skip = 1;
        if (symbols_added == max_symbols) ++t;
        if (t > T) {
            st.pending_skip = t - T;
            t = T;
        }
...
std::vector<int32_t> tdt_greedy(const PredictionNet& pred, const Joint& joint,
    ...
    TdtDecodeState st = tdt_decode_init(pred);
    tdt_decode_frames(pred, joint, enc, T, enc_hidden, durations, st,
                      blank_id, max_symbols, tokens);
    return st.hyp;
}
